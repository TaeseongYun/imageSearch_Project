...
import tech.tsdev.unsplashproject.BuildConfig
...
interface  UnsplashInterface {
    ...
    //검색으로 사진찾는 쿼리(사진은 랜덤)
    @GET("/photos/random?client_id=${BuildConfig.UNSPLASH_KEY}")
    ...
    @GET("{username}?client_id=${BuildConfig.UNSPLASH_KEY}")
    fun getDetailInfo(
    ...
    //최근 사진 불러오게 하는 쿼리
    @GET("?client_id=${BuildConfig.UNSPLASH_KEY}")
    ...
    //사진 다운로드 하게 하는 쿼리
    @GET("{userId}/download?client_id=${BuildConfig.UNSPLASH_KEY}")
